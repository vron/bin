...
def main():
    ...
    # Now create one large zip of the lot
    pp = os.path.split(path)[0]
    zipp = os.path.join(pp, "github bcp vron.zip")
    zipf = zipfile.ZipFile(zipp, 'w', zipfile.ZIP_DEFLATED)
    zipdir(path, zipf)
    zipf.close()

    # Send it to perkeep for storage long term so we have it..
    # upload_pk(zipp)
...
def upload_pk(path):
    subprocess.call(
        [
            ...
            "file",
            "--permanode",
            '--title="Backup of Github ' + datetime.datetime.now().strftime("%Y-%m-%d") + '"',
            "--tag=backup,bcp",
            path,
        ]
    )
